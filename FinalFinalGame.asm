...
;----------------------------------------------------------------------------------------------------------------
;----------------------------------------------------------------------------------------------------------------
;         Note: Run the program only once you have an adequately large console window open
;----------------------------------------------------------------------------------------------------------------
;----------------------------------------------------------------------------------------------------------------
...
.data
    ...
    ; menuBoundary byte "--------------------------------------------------"
    menuBrick byte 219
    menuBoundary byte 50 DUP(219),0
    ...
    score byte 0
    gameOverMsg byte "Snake Princess left you for a longer snake.",0
    retryMsg byte "Press '1' to play again.",0
    ...
    obstacleX byte 55 DUP(?)
    obstacleY byte 55 DUP(?)
    ...
    specialAvailable byte 0
    specialKeyPressed byte 0
    specialLevelActivated byte 0
    obstacleCollisions byte 0
...
SpecialKey1:
    mov al, specialAvailable
    cmp al, 1
    jne NoKey
    mov specialKeyPressed, 1
    jmp NoKey

SpecialKey2:
    mov al, specialKeyPressed
    cmp al, 0
    je NoKey
    call SpecialLevelActivation
...
EatingCheck proc
    ...
    cmp specialLevelActivated, 1
    jne SkipObstacleCheck
    mov esi, 0
    mov ecx, 50
    ...
    mov al, snakeY[esi]
    cmp al, foodY
    je RandomizeFood ; If collision detected, generate new food position
    inc esi
    loop CheckCollisionX
...
ShowCursor proc
    ...
    mov cci.dwSize, 25        ; size (1–100)
    mov cci.bVisible, 1       ; 1 = visible
...
ObstacleCollisionCheck proc
    mov al, specialLevelActivated
    cmp al, 0
    je NoCollision
    mov ecx, 50
    mov esi, 0
...
ObstacleLoop:
    mov al, snakeX[0]        ; head X
    cmp al, obstacleX[esi]
    jne NextObstacle
    mov al, snakeY[0]        ; head Y
    cmp al, obstacleY[esi]
    jne NextObstacle

    mov al, obstacleCollisions
    inc al
    mov obstacleCollisions, al
    cmp al, 4
    jne NoCollision
    call Collision
    ret
...
MenuBox proc
    ...
    ; call gotoxy
    mov ecx, 0
    mov cl, 21 ; Number of rows for walls
    LeftWallLoop:
        ; cmp cl, 1
        ; je DoneLeftWall
        mov dl, 48
...
Settings proc
    ...
    je SetLarge
    mov xBoundary, 70
    mov foodXBoundary, 35
    jmp SetEasy

SetLarge:
    mov xBoundary, 140
    mov foodXBoundary, 70
...
SettingsDone:
    ...
    cmp al, "2"
    jne Return
    mov specialAvailable, 1

Return:
    ret
Settings endp

SpecialLevelActivation proc
    mov specialLevelActivated, 1
    mov obstacleCollisions, 0
    call DrawObstacles
    ret
SpecialLevelActivation endp
...
ObstacleLoop:
    ...
    inc eax
    shl eax, 1 ; Ensure food appears on even X coordinate (since snake moves in steps of 2)
    mov obstacleX[esi], al        ; al = low byte of eax, fine here
    ; New random Y (2 to 2)
    movzx eax, foodYBoundary
    sub eax, 2
    call RandomRange
    add eax, 2 ; Ensure food appears within boundaries (2 to 27)
    mov obstacleY[esi], al        ; same here

    ; Regenerate obstacle if it collides with any current snake segment
    mov al,snakeX[0]
    inc al
    cmp al, obstacleX[esi]
    je ObstacleLoop
...
SnakeNext:
    inc edi
    loop SnakeCollisionCheck

    mov dl, obstacleX[esi]
    mov dh, obstacleY[esi]
    call gotoxy
    mov eax, 219
    call WriteChar
    cmp bl, 0
